...
const OrderHeaderRow = React.memo(function OrderHeaderRow({
  ...
  return (
    <motion.tr
      className={`${emptyCellClass} ${isHovered ? 'has-background-light' : ''}`}
    ...
    >
      <td>
        <label className="checkbox is-size-7">
          <input type="checkbox" className="mr-1 is-size-7" />
    ...
    >
      ...
      <td className={emptyCellClass}>
        <div className="is-flex is-align-items-center is-size-7">
          {item.images?.double && renderImage(item.images.double, 'double', 40, 'image-hover mr-2')}
          {item.images?.love && renderImage(item.images.love, 'love', 30, 'image-hover')}
        </div>
      </td>
    </motion.tr>
  );
...
const ItemInfo: React.FC<ItemInfoProps> = ({ orders: propOrders }) => {
  // ===== CONSTANTS =====
  const emptyCellClass = "has-background-white is-size-7";
  const contentCellClass = "has-background-grey-lighter is-size-7";
  const textClass = "has-text-black is-size-7";
  ...
  const renderTableHeader = () => (
    <thead>
      <tr className="has-background-grey-lighter is-size-7">
        <th className="has-background-grey-lighter has-text-black is-size-7">ALL ORDERS</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">STATUS</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">LINE ITEMS</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">QUANTITY</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">TOTAL COST</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">SHIPPING ADDRESS</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">DETAILS</th>
        <th className="has-background-grey-lighter has-text-black is-size-7">CREATED AT</th>
      </tr>
    </thead>
  );
  ...
  return (
    <div className="card-content">
      ...
      >
        <table className="table is-fullwidth is-bordered mb-0 is-size-7">
          {renderTableHeader()}
          <tbody>
            {renderOrderRows()}
          </tbody>
